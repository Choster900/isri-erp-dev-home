...
const props = defineProps(['propToChangeStateSidebar', 'color']);

// Estado del sidebar
const stateFromSidebar = ref(true); // Sidebar abierto por defecto

// Método para cambiar el estado del sidebar
const changeStateFromSidebar = () => {
    stateFromSidebar.value = !stateFromSidebar.value;
};

// Observador para actualizar el estado del sidebar responsivo
watch(() => props.propToChangeStateSidebar, (newVal) => {
    stateFromSidebar.value = newVal;
});
</script>
<template>
    <div id="sidebar" :class="{
        ...
        '-translate-x-0': stateFromSidebar,
        '-translate-x-64': !stateFromSidebar
    }"
    class="sidebar-style-isri flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-screen overflow-y-scroll shadow-2xl no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 shrink-0 p-4 transition-all duration-200 ease-in-out">
        ...
        <div class="flex justify-center mb-10 pr-3 sm:px-2">
            ...
            <button @click="changeStateFromSidebar"
                class="lg:hidden text-slate-500 hover:text-slate-400" aria-controls="sidebar" aria-expanded="true">
