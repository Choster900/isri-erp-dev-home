        ...
        <div class="bg-white shadow-lg rounded-sm border border-slate-200 relative">
                    ...
                    <div class="mb-4 md:mr-2 md:mb-0 basis-1/4">
                        <div class="relative flex h-8 w-full flex-row-reverse div-multiselect">
                            <Multiselect v-model="tableData.length" @select="getAcquisitionDoc()"
                                @deselect=" tableData.length = 5; getAcquisitionDoc()"
                                @clear="tableData.length = 5; getAcquisitionDoc()" :options="perPage" :searchable="true"
            ...
            <div class="overflow-x-auto">
                <datatable :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy"
                    :searchButton="true" @datos-enviados="handleData($event)" @execute-search="getAcquisitionDoc()">
                    <tbody class="text-sm divide-y divide-slate-200">
                        <tr v-for="doc in acquisition_docs" :key="doc.id_doc_adquisicion">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 td-data-table">
                                <div class="font-medium text-slate-800 ellipsis text-center">{{ doc.razon_social_proveedor
                                }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5  whitespace-nowrap w-px">
                                    ...
                                    <DropDownOptions>
                                        ...
                                        <div class="flex hover:bg-gray-100 py-1 px-2 rounded cursor-pointer"
                                            ...
                                            <div class="w-8 text-red-900"><span class="text-xs">
                                                    <svg :fill="doc.estado_doc_adquisicion == 1 ? '#991B1B' : '#166534'"
                            ...
                            <li v-for="link in links" :key="link.label">
                                <span v-if="(link.label == 'Anterior')"
                                    ...
                                    <div class="flex-1 text-right ml-2">
                                        <a @click="page!=1 ? getAcquisitionDoc(link.url) : ''" class=" btn bg-white border-slate-200 hover:border-slate-300 cursor-pointer
                                  text-indigo-500">
                                ...
                                <span v-else-if="(link.label == 'Siguiente')"
                                    :class="(link.active ? 'inline-flex items-center justify-center rounded-full leading-5 px-2 py-2 bg-white border border-slate-200 text-indigo-500 shadow-sm' : 'inline-flex items-center justify-center leading-5 px-2 py-2 text-slate-600 hover:text-indigo-500 border border-transparent')">
                                    <div class="flex-1 text-right ml-2">
                                        <a @click="hasNext ? getAcquisitionDoc(link.url) : ''" class=" btn bg-white border-slate-200 hover:border-slate-300 cursor-pointer
                                  text-indigo-500">
