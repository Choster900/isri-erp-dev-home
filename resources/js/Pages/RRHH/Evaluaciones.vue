<template>
    ...
    <AppLayoutVue nameSubModule="RRHH - Evaluacion del personal">
        <div class="sm:flex sm:justify-end sm:items-center mb-2">
            <div class="grid grid-flow-col sm:auto-cols-max sm:justify-end gap-2">
                <GeneralButton @click="objectEvaluacionPersonal = []; showModalEvaluacion = !showModalEvaluacion"
                    color="bg-green-700  hover:bg-green-800" text="Evaluar empleado" icon="add" />
            </div>
        </div>
                    ...
                    <tbody class="text-sm divide-y divide-slate-200" v-if="!isLoadinRequest">
                        <tr v-for="(evaluacion, i) in evaluaciones" :key="i">
                            <td class="px-2 first:pl-5 last:pr-5  whitespace-nowrap w-px">
                                <div class="font-medium text-slate-800 text-center text-[8.5pt]">{{ evaluacion.id_empleado
                                }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5  whitespace-nowrap w-px">
                                <div class="font-medium text-slate-800 text-center text-[8.5pt]">{{
                                    evaluacion.codigo_empleado }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5  whitespace-nowrap w-px">
                                <div class="font-medium text-slate-800 text-center text-[8.5pt]">
        ...
        <ModalEvalueaciones v-if="listDependencias != ''" :evaluacionPersonalProp="objectEvaluacionPersonal"
...
import ModalEvalueaciones from '@/Components-ISRI/RRHH/EvaluacionesComponents/ModalEvalueaciones.vue';
import moment from "moment";
export default {
    components: { Head, AppLayoutVue, Datatable, ModalEvalueaciones },
    setup() {
        const {
            links,
            sortBy,
