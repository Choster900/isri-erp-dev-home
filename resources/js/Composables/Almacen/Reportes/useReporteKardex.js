...
import ReporteKardexPdf from "@/pdf/Almacen/ReporteKardexPdf.vue";
import { createApp, h } from 'vue'
import { jsPDF } from "jspdf";
import html2pdf from 'html2pdf.js'
import moment from "moment";
export const useReporteKardex = () => {


    const errors = ref([])
    const isLoadingExport = ref([])
    const isLoadinRequest = ref(false)
    const dataReporteKardex = ref([])

    //Variables a enviar para consulta
    const idProy = ref(null)
    const inProd = ref(null)
    ...
    /**
    * Busca producto por codigo o nombre o cuenta presupuestal
    *
    * @param {string} string - string que se enviara al backend.
    * @returns {Promise<object>} - Objeto con los datos de la respuesta.
    * @throws {Error} - Error al obtener empleados por nombre.
    */
    const handleProducto = async (string) => {
        try {
            // Realiza la búsqueda de empleados
            const response = await axios.post(
                "/get-producto-for-reporte", {
                producto: string,
            }
            );
    ...
    const printPdf = () => {
        ...
        const app = createApp(ReporteKardexPdf, {
            dataKardex: dataReporteKardex.value,
            /* fecha: {
