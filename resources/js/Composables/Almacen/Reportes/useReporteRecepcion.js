...
import ReporteRecepcionPdf from "@/pdf/Almacen/ReporteRecepcionPdf.vue";
export const useReporteRecepcion = (context) => {

    const dateGetRecepcion = ref([])
    const errors = ref([])
    const isLoadinRequest = ref(false)


    const idProy = ref(null)
    ...
    const getInformacionReport = async () => {
        try {
            ...
            dateGetRecepcion.value = [];

            const resp = await axios.post("/get-reporte-recepcion-almacen",
                {
                    idProy: idProy.value,
    ...
    const printPdf = () => {
        ...
        const opt = {
            ...
            margin: 0.5,
            filename: "REPORTE_RECEPCIÓN_" + moment().format("L"),
            //pagebreak: { mode: ['avoid-all', 'css', 'legacy'] },
            image: { type: "jpeg", quality: 0.98 },
            html2canvas: { scale: 3, useCORS: true },
            jsPDF: { unit: "cm", format: "letter", orientation: "portrait" },
        };
        console.log(dateGetRecepcion.value);
        // Crear una instancia de la aplicación Vue para generar el componente quedanPDFVue
        const app = createApp(ReporteRecepcionPdf, {
            dataRecepcion: dateGetRecepcion.value
        });
    ...
    const exportExcel = async () => {
        try {
            try {
                ...
                link.href = url;
                link.setAttribute("download", "REPORTE_ALMACEN_" + moment().format('L') + ".xlsx"); // Nombre del archivo deseado
    ...
    return {
        fechaInicial, fechaFinal, idEstado, idProy,
        getInformacionReport, isLoadinRequest, dateGetRecepcion, exportExcel, printPdf, getOption, errors
