...
export const useEnviarAjusteSalida = (context, getDataToShow, tableData) => {
    ...
    const changeStatusElement = async (elementId, status) => {
        swal({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await axios.post('/change-status-outgoing-adjustment', {
                        id: elementId,
                        status: status
                    });
    ...
    const sendOutgoingAdjustment = async (id) => {
        swal({
            ...
            if (result.isConfirmed) {
                isLoadingTop.value = true;
                try {
                    const response = await axios.post('/send-outgoing-adjustment', {
                        id: id,
                    });
                    useShowToast(toast.success, response.data.message);
                } catch (err) {
